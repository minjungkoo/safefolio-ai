...
@st.cache_data(ttl=3600)  # 1시간 캐시 — API 과호출 방지
def fetch_etf_data(portfolio_key: str) -> dict:
    """
    yfinance로 ETF 실시간 데이터 수집
    - 현재가, 1년 수익률(CAGR), 배당 수익률
    - 포트폴리오 가중 평균 수익률 자동 계산
    """
    try:
...
def calc_account_split(profile, matched_policies):
    ...
    isa_current  = profile.get("isa_current_year", 0)  # 올해 ISA 납입액
    ...
    if not emg_ok:
        ...
        a = remaining * 0.03  # 비상금 충족 시 최소 유지
        alloc["비상금 통장"] = round(a); remaining -= a

    # 이자 비용 (최소한 이자는 내야 함)
    if monthly_interest > 0:
        alloc["대출 이자"] = monthly_interest
        remaining -= monthly_interest

    # 부채 원금 상환 (금리 5% 이상 시 추가)
...
def calc_debt_vs_invest(profile):
    """
    [신규] 부채 상환 vs 투자 수익률 비교
    확정 수익(이자 절감) vs 기대 수익(투자)
    """
    debt_rate = profile.get("debt_rate", 0)
    debt_tot  = profile.get("debt_total", 0)
    if debt_tot == 0:
    ...
    # 세후 실질 투자 수익률 (배당세 15.4% 감안)
    after_tax_return = exp_return * (1 - DIVIDEND_TAX)

    # 100만원 기준 5년 비교
    amount = 1_000_000
    years  = 5
    repay_gain  = amount * debt_rate / 100 * years          # 이자 절감 (확정)
    invest_gain = amount * ((1 + after_tax_return/100)**years - 1)  # 투자 기대 수익

    return {
        "debt_rate": debt_rate,
        "exp_return": exp_return,
        "after_tax_return": after_tax_return,
        "repay_gain": repay_gain,
...
이 포트폴리오({p['name']})만의 장단점을 위 사용자 상황에 맞게 설명하세요.
다른 포트폴리오와 혼동하지 마세요.
...
def get_gemini_chat_response(messages, profile, split_result, selected_portfolio):
    try:
        ...
        system = f"""당신은 한국 사회초년생 자산 관리 AI 'SafeFolio AI'입니다.
아래 사용자 현황을 정확히 파악하고 답변하세요.

[사용자 프로필]
나이: {profile.get('age')}세 / 직업: {profile.get('job_type')} / 성별: {profile.get('gender')}
월 수입: {fmt(profile.get('monthly_income',0))}
월 고정지출: {fmt(profile.get('fixed_expense',0))} / 월 변동지출: {fmt(profile.get('variable_expense',0))}
월 여윳돈: {fmt(split_result.get('surplus',0))}
부채: {'있음 ('+fmt(profile.get('debt_total',0))+', 금리 '+str(profile.get('debt_rate',0))+'%, 월이자 '+fmt(split_result.get('monthly_interest',0))+')' if profile.get('debt_total',0)>0 else '없음'}
비상금: {fmt(profile.get('emergency_current',0))} / 목표: {fmt(split_result.get('emergency_target',0))} ({'충족' if split_result.get('emergency_ok') else '미충족'})
3년 목표: {fmt(profile.get('goal_3y',0))}
...
[통장 쪼개기 알고리즘 원칙 (질문 시 설명용)]
1. 비상금 목표 = 총 생활비(고정+변동) × 6개월
2. 부채 금리 ≥ 5% 시 투자보다 상환 우선
3. 청년미래적금 조건 충족 시 월 50만원 우선 배정
4. 이자 비용은 반드시 먼저 차감 후 배분
5. 성과급: 비상금 미충족 시 전액 비상금 / 충족 시 70% 투자·30% 자유

[답변 원칙]
1. 2026년 한국 세법 기준으로 정확하게
2. 위 수치를 적극 활용해 구체적으로 (숫자 포함)
3. 사회초년생 눈높이로 쉽게
4. 투자 권유 아닌 교육·정보 제공
5. 간결하게, 필요시 이모지 활용
"""
...
def page_onboarding():
    ...
    with st.form("onboarding_form"):
        ...
        # [수정] 고정지출 + 변동지출 분리
        st.markdown("**지출 정보**")
        st.caption("고정지출: 주거비·통신비·보험료 등 매달 고정적으로 나가는 돈\n변동지출: 식비·교통비·여가 등 달마다 달라지는 돈")
        c1, c2, c3 = st.columns(3)
        ...
        isa_current_year  = c2.number_input("올해 ISA 납입액 (원)",
            ...
            help="올해 이미 ISA에 넣은 금액 (연간 한도 ₩2,000만원 체크용)")
...
def page_dashboard():
    ...
    with left:
        st.markdown("**💰 자산 입력**")
        st.caption("ISA 계좌 안에 담긴 ETF는 'ISA-ETF/주식'에 입력하세요. 일반 계좌 ETF와 분리됩니다.")
        with st.form("asset_form"):
            st.markdown("*절세 계좌*")
            c1, c2 = st.columns(2)
            isa_etf     = c1.number_input("ISA — ETF/주식 (원)",
                min_value=0, value=0, step=100_000, format="%d")
            isa_deposit = c2.number_input("ISA — 예적금/채권 (원)",
                min_value=0, value=0, step=100_000, format="%d")
            youth_sav   = st.number_input("청년미래적금 (원)",
                min_value=0, value=0, step=100_000, format="%d")

            st.markdown("*일반 계좌*")
            c1, c2 = st.columns(2)
            stock_gen   = c1.number_input("일반계좌 — 주식/ETF (원)",
                min_value=0, value=0, step=100_000, format="%d")
            savings_gen = c2.number_input("일반계좌 — 예적금 (원)",
                min_value=0, value=0, step=100_000, format="%d")
            emergency   = st.number_input("비상금 통장 (원)",
                min_value=0, value=int(p.get("emergency_current",0)),
            ...
            if debt_yn2 == "있음":
                c1, c2 = st.columns(2)
                debt_total2 = c1.number_input("총 부채 금액 (원)",
                    min_value=0, value=int(p.get("debt_total",10_000_000)),
                    step=1_000_000, format="%d", key="dt2")
            ...
            for i, m in enumerate(months):
                v = exp_cols[i%4].number_input(m, min_value=0,
                    value=total_exp_default, step=100_000, format="%d", key=f"exp_{i}")
                expense_list.append(v)

            save_btn = st.form_submit_button("📊 대시보드 업데이트",
                use_container_width=True, type="primary")

        if save_btn:
            st.session_state.assets = {
                "ISA-ETF/주식":      isa_etf,
                "ISA-예적금/채권":   isa_deposit,
                "청년미래적금":      youth_sav,
                "일반계좌-주식/ETF": stock_gen,
                "일반계좌-예적금":   savings_gen,
                "비상금 통장":       emergency,
            }
            ...
            st.session_state.profile["emergency_ok"] = emergency >= total_exp * 6
            st.session_state.profile["debt_total"]   = debt_total2
            st.session_state.profile["debt_rate"]    = debt_rate2
            st.session_state.profile["total_assets"] = sum([
                isa_etf, isa_deposit, youth_sav, stock_gen, savings_gen, emergency])
            st.rerun()
    ...
    with right:
        ...
        m2.metric("순 자산 (Net Worth)", fmt(net_worth))
        m3.metric("3년 목표 달성률",
                  f"{min(net_worth/goal*100,999):.0f}%" if goal else "—")

        # 자산 분포 도넛
        nonzero = {k:v for k,v in assets.items() if v > 0}
        if nonzero:
            fig = donut_chart(list(nonzero.keys()), list(nonzero.values()),
                title="자산 분포",
                colors=["#4C9BE8","#1ABC9C","#F1C40F","#E74C3C","#9B59B6","#2ECC71"],
                center_text=fmt(total_assets))
        ...
        if debt_tot > 0:
            ...
            if monthly_int > 0:
                st.info(f"💸 월 이자 비용: **{fmt(monthly_int)}** "
                        f"(연 {debt_rate2}% / 연간 {fmt(monthly_int*12)})")
  ...
  {conflict_note}{policy_note}{action_html}
    ...
    with tab2:
        ...
        if split.get("fixed_warning"):
            st.warning(f"⚠️ 총 지출 비율 {split['fixed_ratio']*100:.0f}% — 60% 초과, 지출 구조 점검 필요")
        if not split.get("emergency_ok"):
            st.warning(f"📌 비상금 목표 {fmt(split['emergency_target'])} (월 생활비 × 6개월) 중 "
                       f"{fmt(p.get('emergency_current',0))} 보유. 부족분 {fmt(split['emergency_gap'])}")

        # ISA 한도 안내
        isa_rem = split.get("isa_remaining", ISA_ANNUAL_LIMIT)
        if isa_rem < ISA_ANNUAL_LIMIT:
            st.info(f"📋 ISA 올해 잔여 납입 한도: **{fmt(isa_rem)}** "
                    f"(연간 {fmt(ISA_ANNUAL_LIMIT)} 중 {fmt(ISA_ANNUAL_LIMIT - isa_rem)} 납입 완료)")
        ...
        if dvi:
            ...
            c1, c2 = st.columns(2)
            c1.metric("💰 부채 상환 시 이자 절감 (확정)",
                      fmt(dvi["repay_gain"]),
                      delta=f"금리 {dvi['debt_rate']}% 확정 수익")
            c2.metric("📈 투자 시 기대 수익 (변동)",
                      fmt(dvi["invest_gain"]),
                      delta=f"세후 기대수익률 {dvi['after_tax_return']:.1f}%")
            if dvi["prefer_repay"]:
                st.error(f"🏦 **상환 우선 권장** — 대출금리({dvi['debt_rate']}%)가 "
                         f"세후 투자 기대수익률({dvi['after_tax_return']:.1f}%)보다 높아요.")
            else:
                st.success(f"📊 **투자 병행 가능** — 세후 투자 기대수익률({dvi['after_tax_return']:.1f}%)이 "
                           f"대출금리({dvi['debt_rate']}%)보다 높아요.")
        ...
        if st.session_state.get("risk_profile"):
            ...
            if goal_sim["achievable"]:
                ...
                st.warning(f"⚠️ 3년 후 예상 자산 {fmt(goal_sim['final_3y'])} — "
                           f"목표까지 {fmt(goal_sim['shortfall'])} 부족. 월 저축액 증액을 고려하세요.")
    ...
    with tab3:
        ...
        for key in sorted_keys:
            ...
            with st.expander(
                ...
                # ── 실시간 ETF 데이터 (yfinance 에이전트 기능) ──
                realtime_key = f"realtime_{key}"
                if realtime_key not in st.session_state:
                    st.session_state[realtime_key] = None

                if st.button("🔴 실시간 ETF 데이터 불러오기",
                             key=f"fetch_{key}",
                             help="yfinance로 각 ETF의 현재가·1년수익률·배당률을 자동 수집해요"):
                ...
                if rd:
                    if rd.get("success"):
                        ...
                        # 가중 평균 수익률로 포트폴리오 지표 업데이트
                        w_ret = rd["weighted_return"]
                        w_div = rd["weighted_div"]
                        rc1, rc2, rc3 = st.columns(3)
                        rc1.metric("포트폴리오 가중 평균 1년 수익률",
                                   f"{w_ret:+.1f}%",
                        ...
                        if w_ret > 0:
                            st.success(f"📊 실제 1년 수익률 기준 이 포트폴리오의 가중 평균 수익률은 **{w_ret:+.1f}%** 예요.")
                        elif w_ret < -5:
                            st.warning(f"⚠️ 최근 1년 수익률이 **{w_ret:.1f}%** 로 부진해요. 장기 관점으로 접근하세요.")
                    else:
                        st.error(f"데이터 수집 실패: {rd.get('error','알 수 없는 오류')}")
                        st.caption("네트워크 연결 또는 Yahoo Finance 접근을 확인해주세요.")
...
def page_chatbot():
    ...
    if prompt := st.chat_input("질문을 입력하세요 (예: 왜 비상금을 6개월치로 잡아야 해요?)"):
